...
- A minimal suggested test command when the recommended tests collapse cleanly enough.
- Heuristic tests still visible, but clearly secondary.

**Implemented:** `build_minimal_context` now renders its test section as focused verification categories: direct regression tests, contract/API forwarding tests, integration-level verification, and heuristic shield tests. It also emits a narrow `dotnet test` suggestion when the non-heuristic candidates resolve cleanly enough to one seam.
