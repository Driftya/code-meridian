...
public sealed partial class DocumentIngestionService(CodeMeridianClient client, ILogger<DocumentIngestionService> logger)
{
    public async Task<DocumentStats> IngestAsync(
        FileInfo[] files,
        string projectContext,
        string rootPath,
        bool inferRelatedReferences = false,
        CancellationToken cancellationToken = default)
    {
        ...
        foreach (var file in files)
        {
            try
            {
                ...
                var relatedDocuments = ExtractDocumentReferences(content);
                var relatedNodes = inferRelatedReferences ? ExtractCodeFileReferences(content, projectContext) : [];
    ...
    private static List<string> ExtractDocumentReferences(string content)
    {
    ...
    private static List<string> ExtractCodeFileReferences(string content, string projectContext)
    {
        var references = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
