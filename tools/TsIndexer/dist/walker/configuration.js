...
function findConfigurationUsages(sourceFile) {
    const results = [];
...
function readEnvironmentPropertyAccess(propertyAccess) {
...
function readEnvironmentElementAccess(elementAccess) {
...
function readEnvironmentDestructuring(declaration) {
    ...
    if (sourceText !== 'process.env' && sourceText !== 'import.meta.env')
        return [];
...
function readEnvironmentSchemaBinding(binary) {
    if (binary.getOperatorToken().getKind() !== SyntaxKind.EqualsToken)
        return undefined;
    const leftText = binary.getLeft().getText();
    if (!leftText.endsWith('.env') && leftText !== 'env')
        return undefined;
    const right = binary.getRight();
    if (!right.isKind(SyntaxKind.CallExpression))
        return undefined;
    const callee = right.getExpression().getText();
    if (!callee.endsWith('.object') && !callee.endsWith('.pick'))
        return undefined;
...
function deriveOptionsType(leftText) {
...
function normalizeConfigurationKey(rawKey) {
