...
function collectRouteNodes(sourceFile, rootPath, projectName, nodes, knownIds) {
...
function collectRouteEdges(sourceFile, rootPath, projectName, edges, knownIds) {
    const routes = extractHttpRouteCalls(sourceFile, rootPath, projectName, knownIds);
    for (const route of routes) {
        edges.push({
...
function extractHttpRouteCalls(sourceFile, rootPath, projectName, knownIds) {
    const relPath = path.relative(rootPath, sourceFile.getFilePath()).replace(/\\/g, '/');
...
function resolveHttpRouteCall(call, relPath, projectName, knownIds, context) {
...
function resolveFetchMethod(call) {
    const initArg = call.getArguments()[1];
    if (!initArg || initArg.getKind() !== SyntaxKind.ObjectLiteralExpression)
        return undefined;
...
function resolveRequestConfig(configArg, context) {
    ...
    if (!urlInitializer || !methodInitializer)
        return undefined;
    ...
    if (!resolvedRoute || !method)
        return undefined;
    return { ...resolvedRoute, method };
}
function buildRouteContext(sourceFile) {
    const stringConstants = new Map();
    const objectRoutes = new Map();
...
function resolveRouteExpression(node, context) {
...
function resolveRouteLiteral(node) {
...
function resolveStringLiteralValue(node) {
...
function normalizeRouteTemplate(template) {
...
function buildApiEndpointId(projectName, method, normalizedRoute) {
    return `${projectName}::ApiEndpoint::${method} ${normalizedRoute}`;
}
function resolveRouteSourceId(call, projectName, relPath, knownIds) {
