...
for (const node of nodes) {
    try {
        ...
        nodeErrors++;
        if (nodeErrors <= 5)
            console.warn(`  warn: node ${node.id}: ${e}`);
    }
}
...
for (const edge of edges) {
    try {
        ...
        edgeErrors++;
        if (edgeErrors <= 5)
            console.warn(`  warn: edge ${edge.sourceId} → ${edge.targetId}: ${e}`);
    }
}
...
function readFilesList(filePath) {
    return fs
...
function resolveDocumentFiles(rootPath, files) {
    if (files) {
        return files
...
function enumerateFiles(rootPath) {
    const results = [];
    const pending = [rootPath];
    while (pending.length > 0) {
        const current = pending.pop();
        if (isIgnoredPath(rootPath, current))
            continue;
...
function resolveInputFile(rootPath, filePath) {
