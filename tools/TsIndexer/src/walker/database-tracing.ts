...
import type { DatabaseTracingOptions, DatabaseTracingPreset } from './database-tracing-options.js';

type DatabaseOperationType = 'Reads' | 'Writes';

interface DatabaseUsageCandidate {
  sourceId: string;
  lineNumber: number;
  recognizerId: string;
  provider: string;
...
export function collectDatabaseTracingNodes(
...
export function collectDatabaseTracingEdges(
  sourceFile: SourceFile,
  rootPath: string,
  projectName: string,
  edges: CodeEdgeDto[],
...
function findDatabaseUsages(
  sourceFile: SourceFile,
  projectName: string,
  relPath: string,
  options: DatabaseTracingOptions,
): DatabaseUsageCandidate[] {
  ...
  const usages: DatabaseUsageCandidate[] = [];
...
function tryRecognizeUsage(
  call: CallExpression,
  preset: DatabaseTracingPreset,
  projectName: string,
  relPath: string,
  maxTablesPerOperation: number,
  importedModules: Set<string>,
): DatabaseUsageCandidate | undefined {
...
function extractKnexTableName(receiver: Node | undefined): string | undefined {
  if (!receiver) {
    return undefined;
  }
...
function findPrismaModelName(receiver: Node | undefined): string | undefined {
  if (!receiver?.isKind(SyntaxKind.PropertyAccessExpression)) {
    return undefined;
  }
...
function resolveStatementFromArguments(call: CallExpression, preset: DatabaseTracingPreset): string | undefined {
...
function resolveStatementText(node: Node | undefined, depth = 0): string | undefined {
  if (!node || depth > 6) {
    return undefined;
  }
...
function buildOperationNodeId(
  projectName: string,
  sourceId: string,
  relPath: string,
  lineNumber: number,
  usage: Pick<DatabaseUsageCandidate, 'provider' | 'methodName' | 'operationType' | 'tables'>,
): string {
...
function supportsLanguage(preset: DatabaseTracingPreset, language: string): boolean {
...
function supportsMethod(methodName: string, preset: DatabaseTracingPreset): boolean {
  return [...preset.readMethods, ...preset.writeMethods]
    .some(candidate => candidate.toLowerCase() === methodName.toLowerCase());
}

function matchesProviderHints(receiverText: string, preset: DatabaseTracingPreset, importedModules: Set<string>): boolean {
...
function getOperationType(methodName: string, preset: DatabaseTracingPreset): DatabaseOperationType | undefined {
...
function getMemberName(call: CallExpression): string | undefined {
...
function getReceiver(call: CallExpression): Node | undefined {
  const expression = call.getExpression();
  return expression.isKind(SyntaxKind.PropertyAccessExpression) ? expression.getExpression() : undefined;
}

function inferCypherOperationType(statementText: string): DatabaseOperationType | undefined {
...
function looksLikeCypher(statementText: string | undefined): statementText is string {
  return typeof statementText === 'string' && (cypherReadRegex.test(statementText) || cypherWriteRegex.test(statementText));
}

function extractCypherEntities(statementText: string, maxTablesPerOperation: number): string[] {
...
function dedupeUsages(usages: DatabaseUsageCandidate[]): DatabaseUsageCandidate[] {
  const seen = new Set<string>();
  const deduped: DatabaseUsageCandidate[] = [];
...
function isFunctionValuedVariable(variable: VariableDeclaration | undefined): variable is VariableDeclaration {
  if (!variable) {
    return false;
  }
