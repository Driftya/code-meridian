...
- [x] [P2 - Add Context-Pack Test Recommendation Pruning](docs/features/50-add-context-pack-test-recommendation-pruning.md) - `build_minimal_context` should reuse focused verification categories instead of a broad related-tests bucket.
- [x] [P2 - Prune Related-Knowledge Result Noise](docs/features/51-prune-related-knowledge-result-noise.md) - Collapse duplicate lexical matches and suppress weak keyword-only result soup.
- [x] [P2 - Prune Optional Context Workflow Steps By Default](docs/features/52-prune-optional-context-workflow-steps.md) - Narrow tasks should not default to broad optional-step itineraries.
- [x] [P2 - Add Tool Dependency Impact Matrix](docs/features/53-add-tool-dependency-impact-matrix.md) - Make cross-tool coupling explicit so feature and refactor work can verify affected reports, evaluators, MCP tools, and regression suites.
...
- [x] [P2 - Add Test Target Precision](docs/features/41-add-test-target-precision.md) - Suggest the tests most likely to change or run, not every loosely related shield.
...
- [x] [P2 - Expand Bridge Node Detection Into Risky Core Analysis](docs/features/25-add-bridge-node-detection.md) - Add centrality, articulation-point, and bridge-edge scoring across Roslyn and TsIndexer graphs.
- [x] [P2 - Add Structural Implementation Pattern Search](docs/features/54-add-structural-implementation-pattern-search.md) - Find existing endpoint/service/repository/test shapes that match a new requested feature.
...
- [x] [P2 - Expand Dependency Smell Paths With Forbidden-Dependency Presets](docs/features/33-add-dependency-smell-paths.md) - Add named architecture checks backed by variable-length path search.
- [x] [P2 - Expand Semantic Graph Hybrid Search With Graph Re-Ranking](docs/features/32-add-semantic-graph-hybrid-search.md) - Combine full-text, vector search, graph expansion, and graph-evidence reranking.
...
- [x] [P1 - Add Derived Edit-Surface Credit For Extraction Refactors](docs/features/56-add-derived-edit-surface-credit-for-extraction-refactors.md) - `evaluate-session` should not treat newly created collaborator files as unrelated work when they are clearly derived from an exact suggested target or responsibility slice.
